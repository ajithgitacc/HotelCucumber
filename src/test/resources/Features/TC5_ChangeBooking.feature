Feature: Verifying OMR Hotel Booking change booking module automation
@Change
  Scenario Outline: Modify the Checkin date for new booking
  ...
  Scenario Outline: Modify the checkin date for existing order id
    ...
    Examples: 
      | userName                      | password | existingOrderId | modifyCheckIn |
      | ajithkumarmoorthy11@gmail.com | Ajith@31 | #RRZFH49408     | 2024-05-28    |

  
  Scenario Outline: Modify the checkin date for first displayed order id
    Given User is on the omr page
    When User login "<userName>" and "<password>"
    Then User should verify after login success message "Welcome AJITHKUMAR"
    When User navigate into My Bookings page
    Then User should verify after navigate to My Booking page success message as "Bookings"
    When User modify the checkin date for first order id "<modifyCheckIn>"
    Then User should verify after modify checkin date booking success message "Booking updated successfully"

    Examples: 
      | userName                      | password | modifyCheckIn |
      | ajithkumarmoorthy11@gmail.com | Ajith@31 | 2024-05-28    |
